...
from evaluation.similarity_metrics import rouge_l_simScore, sentence_bert_simScore
from data.pickle_io import append_to_pickle
...
def evaluate_and_store(
    ...
    selected_token_vecs: torch.Tensor,
    index_map: Dict[int, int],
    batch_start_idx: int,
    tokenizer: PreTrainedTokenizer
) -> Dict[str, Any]:
    """
    Evaluate generated answers, compute similarity scores, and store batch results.

    Parameters
    ----------
    batch : List[Dict[str, Any]]
        List of batch examples.
    prompts : List[str]
    ...
    output_ids : torch.Tensor
        Generated token IDs.
    selected_token_vecs : torch.Tensor
        Last-token activation vectors.
    index_map : Dict[int, int]
        Mapping from batch indices to original dataset indices.
    batch_start_idx : int
        Index of the first sample in this batch.
    tokenizer : PreTrainedTokenizer
    ...
    -------
    Dict[str, Any]
        Dictionary containing all relevant batch results.
    """
    batch_dataset_ids = []
    batch_dataset_original_idx = []
    batch_answers = []
    batch_gt_answers = []
    batch_is_correct = []
    batch_rouge_scores = []
    ...
    for j in range(len(prompts)):
        ...
        generated_answer_ids = output_ids[j][prompt_len:]
        generated_answer = tokenizer.decode(generated_answer_ids, skip_special_tokens=True).strip()
...
def tokenize_prompts(
    ...
    """
    Tokenize prompts and move tensors to the specified device.
    ...
    prompts : List[str]
        List of input prompts.
    tokenizer : PreTrainedTokenizer
        Tokenizer to process input prompts.
    device : torch.device
        Device (e.g., torch.device("cuda") or torch.device("cpu")) to move tensors to.

    Returns
    -------
    inputs : BatchEncoding
        Tokenized input tensors ready for model inference.
    """
...
def extract_last_token_activations(
    ...
    """
    Extract the last non-padding token vector for each input in the batch.

    Parameters
    ----------
    selected_layer : torch.Tensor
        Output tensor of the selected model layer (shape: batch_size x seq_len x hidden_size).
    attention_mask : torch.Tensor
    ...
    -------
    torch.Tensor
        Activation vectors of the last non-padding token for each input (shape: batch_size x hidden_size).
    """
    last_indices = (attention_mask.sum(dim=1) - 1).to(device)
    batch_indices = torch.arange(selected_layer.size(0), device=device)
    return selected_layer[batch_indices, last_indices]
'''